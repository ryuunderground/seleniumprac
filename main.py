...
class GoogleKeywordScreenshooter:
    ...
    def search_and_screen_capture(self):
        ...
        try:
            ...
            pass
        #결과 모으기
        search_results = self.browser.find_elements(By.CLASS_NAME, "TzHB6b")
        search_results.pop()
        #결과 추려내기, 제목만
        os.mkdir(f"{self.screenshots_dir}/{self.keyword}")
        for index, search_result in enumerate(search_results):
            search_result.screenshot(f"{self.screenshots_dir}/{self.keyword}/{self.keyword} x {index}.png")
            
    def execute(self):
        #브라우저 끄기
        self.browser.quit()

mmummumum = GoogleKeywordScreenshooter("Nanashi Mumei", "screenshots")
mmummumum.search_and_screen_capture()
mmummumum.execute()
gumba =  GoogleKeywordScreenshooter("Gawr Gura", "screenshots")
gumba.search_and_screen_capture()
gumba.execute()
Axe = GoogleKeywordScreenshooter("Hochimachi suisei", "screenshots")
Axe.search_and_screen_capture()
Axe.execute() """

Kronii = GoogleKeywordScreenshooter("Ouro Kronii", "screenshots")
Kronii.search_and_screen_capture()